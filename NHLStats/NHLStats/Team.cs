...
namespace NHLStats
{
    public class Team
    {
        ...
        public static string GetTeamID(string teamAbbreviation)
        {
            string teamID = "0";
            ...
            teamsAPICall = NHLAPIServiceURLs.teams;

            // Get team list from NHL API
            var json = DataAccessLayer.ExecuteAPICall(teamsAPICall);

            // Populate the raw JSON to a property
            var teamsJson = JArray.Parse(json.SelectToken("data").ToString());




            foreach (JToken team in teamsJson)
            {
                if (team.SelectToken("triCode").ToString() == teamAbbreviation)
                {
                    teamID = team.SelectToken("id").ToString();
                    break;
                }
